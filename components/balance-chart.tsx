    ...
    <Card className="border-none bg-indigo-900 text-white shadow-lg">
      <CardHeader className="flex flex-row items-start sm:items-center justify-between border-none gap-4 pb-2">
        <div className="min-w-0 flex-1">
          <CardTitle className="text-base sm:text-lg font-medium text-white/90">Total Balance</CardTitle>
          <div className="mt-1 flex items-center gap-2">
            <span className="text-xl sm:text-2xl font-bold truncate">₱{(current ?? 0).toLocaleString()}</span>
            <span className={`hidden sm:flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${growth >= 0 ? 'bg-white/20 text-white' : 'bg-red-500/20 text-red-100'}`}>
