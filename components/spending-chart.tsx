"use client";
import { Bar, BarChart, ResponsiveContainer, Tooltip, XAxis } from "recharts";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { ArrowUpRight, Calendar } from "lucide-react";

export function SpendingChart({ data, total }: { data: any[], total?: number }) {
  // Mock data if empty
  const chartData = data?.length ? data : [
    { day: "Sun", amount: 12000 },
    { day: "Mon", amount: 25000 },
    { day: "Tue", amount: 18000 },
    { day: "Wed", amount: 35000 }, // Highlighted in design
    { day: "Thu", amount: 22000 },
    { day: "Fri", amount: 28000 },
    { day: "Sat", amount: 15000 },
  ];

  return (
    <Card className="border-none bg-gray-900 text-white shadow-lg">
      <CardHeader className="flex flex-row items-start sm:items-center justify-between border-none gap-4 pb-2 sm:pb-2">
        <div className="min-w-0 flex-1">
          <CardTitle className="text-base sm:text-lg font-medium text-white/90">Spending</CardTitle>
          <div className="mt-1 flex items-center gap-2">
            <span className="text-xl sm:text-2xl font-bold">₱{(total ?? 0).toLocaleString()}</span>
            <span className="hidden sm:flex items-center rounded-full bg-white/20 px-2 py-0.5 text-xs font-medium text-white">
              <ArrowUpRight className="mr-1 h-3 w-3" />
              3.4%
            </span>
          </div>
        </div>
        <button className="flex items-center gap-1.5 sm:gap-2 rounded-lg bg-white/10 px-2 sm:px-3 py-1.5 text-xs font-medium text-white hover:bg-white/20 shrink-0">
          <span className="hidden sm:inline">This Week</span>
          <span className="sm:hidden">Week</span>
          <Calendar className="h-3 w-3" />
        </button>
      </CardHeader>
      <CardContent>
        <div className="h-[160px] sm:h-[180px] md:h-[200px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} barSize={8}>
              <XAxis
                dataKey="day"
                axisLine={false}
                tickLine={false}
                tick={{ fill: 'rgba(255,255,255,0.6)', fontSize: 10 }}
                dy={10}
              />
              <Tooltip
                cursor={{ fill: 'rgba(255,255,255,0.1)' }}
                contentStyle={{ backgroundColor: '#fff', borderRadius: '8px', border: 'none', color: '#000', fontSize: '12px' }}
                formatter={(value: number) => [`₱${value.toLocaleString()}`, "Spent"]}
              />
              <Bar
                dataKey="amount"
                fill="rgba(255,255,255,0.9)"
                radius={[4, 4, 4, 4]}
              />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </CardContent>
    </Card>
  );
}
